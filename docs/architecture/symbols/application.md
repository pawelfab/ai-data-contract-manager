...
| `signal_binder.py` | `SignalBinder.bind` | Bind a signal only when one authorized path declares its concept. |
| `preference_expander.py` | `PreferenceExpander.expand` | Create candidates for every current authorized path declaring an active preference concept. |
| `candidate_resolver.py` | `CandidateResolver.resolve` | Select by explicit/effective priority, origin/scope rank, revision/sequence, and confidence. |
