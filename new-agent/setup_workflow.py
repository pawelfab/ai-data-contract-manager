...
def main() -> int:
    ...
    if run(["python", "scripts/agent/validate_setup.py"]) != 0:
        return 1
    if run(["python", "scripts/agent/repo_inventory.py"]) != 0:
        return 1
    if args.install_hooks and run(["python", "scripts/agent/install_git_hooks.py"]) != 0:
        return 1
