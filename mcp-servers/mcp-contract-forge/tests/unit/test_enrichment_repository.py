...
def test_system_enrichment_requires_matching_context(tmp_path: Path):
    ...
    assert resolve_enrichment({}, rules, EnrichmentContext(), eligible_paths={"/silver/dataset"}) == []
    suggestions = resolve_enrichment({}, rules, EnrichmentContext(source_system="SAP"), eligible_paths={"/silver/dataset"})
...
def test_user_repository_overrides_system_repository(tmp_path: Path):
    ...
    suggestions = resolve_enrichment({}, rules, context, eligible_paths={"/silver/dataset"})
...
def test_global_template_copy_and_interpolation(tmp_path: Path):
    ...
    suggestions = resolve_enrichment(doc, rules, EnrichmentContext(), eligible_paths={"/metadata/id", "/rawData/gcsBucketPath"})
...
def test_path_pattern_applies_only_to_current_eligible_paths(tmp_path: Path):
    ...
    rules = JsonEnrichmentRepository(path).get_rules(EnrichmentContext())
    suggestions = resolve_enrichment(doc, rules, EnrichmentContext(), eligible_paths={"/some/source/systemZrodlowy", "/other/value"})
