...
def main() -> int:
    ...
    for key in REPOSITORY_PATH_KEYS:
        value = config.get(key)
        if not isinstance(value, str) or not is_repo_relative_path(value):
            errors.append(f"Config path must stay inside the repository: {key}={value!r}")
